...
<form action="<?php echo U('News/edit');?>" method="post" name="myform" id="myform" enctype="multipart/form-data" style="margin-top:10px;">
  <div class="pad-10">
  <div class="col-tab">
  ...
  <div id="div_setting_1" class="contentList pad-10">
    <table width="100%" cellpadding="2" cellspacing="1" class="table_form">
	  ...
	  <tr>
        <th width="100">来源 :</th>
        <td><input type="text" name="source" id="source" class="input-text validate[required]" size="60" value="<?php echo ($article["source"]); ?>"></td>
      </tr>
	  <tr>
        <th width="100">所属分类 :</th>
        <td><select name="type" >
		      <option value="1" <?php if($article['type'] == 1): ?>selected="selected"<?php endif; ?>>展会新闻</option>
			  <option value="2" <?php if($article['type'] == 2): ?>selected="selected"<?php endif; ?>>展商速递</option>
			  <option value="3" <?php if($article['type'] == 3): ?>selected="selected"<?php endif; ?>>视频报道</option>
		</select></td>
      </tr>
	  <tr>
        <th width="100">所属主题 :</th>
        <td><select name="theme" >
		  <?php if(is_array($catetype)): $i = 0; $__LIST__ = $catetype;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($val['id']); ?>" <?php if($article['theme'] == $val['id']): ?>selected="selected"<?php endif; ?>><?php echo ($val['kind']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select></td>
      </tr>
	   <tr id="info1">
       <th width="100">图片 :</th> 
