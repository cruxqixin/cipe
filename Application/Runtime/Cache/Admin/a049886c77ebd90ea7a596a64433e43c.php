...
<div class="pad-lr-10" >
    <form id="myform" name="myform" action="<?php echo U('Media/cateDelete');?>" method="post" >
    ...
    <table width="100%" cellspacing="0">
        ...
        <?php if(is_array($article_cate)): $i = 0; $__LIST__ = $article_cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
           ...
           <td align="center"><input  type="text" class="input-text-c input-text" value="<?php echo ($val["ord"]); ?>" size="4" name="orders[<?php echo ($val["id"]); ?>]"></td>
            <td align="center"><a class="blue" href="<?php echo U('Media/cateEdit',array('id'=>$val['id']));?>">编辑</a></td>
   ...
   <input type="submit" class="button" name="order" onclick="document.myform.action='<?php echo U('Media/cateOrder');?>'" value="排序" style="float:left;"/>
    </div>
